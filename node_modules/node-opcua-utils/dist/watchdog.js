...
function _getCurrentSystemTick() {
    if (process && process.hrtime) {
        ...
        const n = h[1] / 1000000;
        node_opcua_assert_1.assert(n <= 1000);
        return h[0] * 1000 + n;
    }
    else {
...
function keepAliveFunc() {
    node_opcua_assert_1.assert(this._watchDog instanceof WatchDog);
    if (!this._watchDogData || !this._watchDog) {
        throw new Error("Internal error");
    }
    node_opcua_assert_1.assert(typeof this._watchDogData.key === "number");
...
class WatchDog extends events_1.EventEmitter {
    ...
    addSubscriber(subscriber, timeout) {
        ...
        node_opcua_assert_1.assert(typeof timeout === "number", " invalid timeout ");
        node_opcua_assert_1.assert(typeof subscriber.watchdogReset === "function", " the subscriber must provide a watchdogReset method ");
        node_opcua_assert_1.assert(typeof subscriber.keepAlive !== "function" || subscriber.keepAlive === WatchDog.emptyKeepAlive);
        this._counter += 1;
        ...
        if (this.subscriberCount === 1) {
            node_opcua_assert_1.assert(this._timer === null);
            this._start_timer();
        }
        node_opcua_assert_1.assert(this._timer !== null);
        return key;
    ...
    removeSubscriber(subscriber) {
        ...
        node_opcua_assert_1.assert(typeof subscriber._watchDogData.key === "number");
        node_opcua_assert_1.assert(typeof subscriber.keepAlive === "function");
