...
class ClientSecureChannelLayer extends events_1.EventEmitter {
    ...
    _on_message_received(response, msgType, requestId) {
        ...
        /* istanbul ignore next */
        if (doTraceMessage) {
            console.log(chalk.cyan.bold(node_opcua_utils_1.timestamp(), "  <<<<<< _on_message_received "), requestId, response.schema.name.padStart(30), response.responseHeader.serviceResult.toString());
        }
        const requestData = this._requests[requestId];
        /* istanbul ignore next */
        if (!requestData) {
            console.log(chalk.cyan.bold("xxxxx  <<<<<< _on_message_received for unknown or timeout request "), requestId, response.schema.name, response.responseHeader.serviceResult.toString());
